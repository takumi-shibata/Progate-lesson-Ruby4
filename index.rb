...
# 8.ファイルの分割(分割先のファイルで定義されているクラスを使用するにはrequireする必要がある。jsと似ている、詳しくはjs5のリポジトリ参照)
...
# 9.インスタンスの配列(クラスから生成したインスタンスも、配列の要素にすることが可能)
require "./menu"
...
# 10.入力の受け取り(文字列や数字をコンソールで入力し、その入力した値を変数に代入する事ができる。jsではパッケージ、詳しくはjs5のリポジトリ参照)
